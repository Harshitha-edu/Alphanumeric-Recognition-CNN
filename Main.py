...
main = tkinter.Tk()
main.title("Character Recognition") #designing main screen
...
def uploadDataset():
    ...
    text.insert(END,"Different Characters Found in Dataset : "+str(labels)+"\n\n") 
...
def predict():
    ...
    cv2.putText(img, f'Character Recognition Predicted as: {disease_label}', (10, 25),  cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)
    cv2.imshow(f'Character Recognition Predicted as: {disease_label}', img)
    cv2.waitKey(0)
...
text_frame = Frame(main, bg='#f4f6f9')
...
# ---- Buttons ----
button_frame = Frame(main, bg='#f4f6f9')
button_frame.pack(pady=20)

button_style = {
    "bg": "#3b82f6",  # Modern blue
    "fg": "white",
    "activebackground": "#2563eb",  # Darker on hover
...
# Button layout
buttons = [
    ("Upload Kaggle Dataset", uploadDataset),
    ("Extract Texture & GLCM Features", featuresExtraction),
    ("Run SVM Algorithm", runSVM),
    ("Run Proposed CNN Algorithm", runCNN),
    ("Comparison Graph", graph),
    ("Character Recognition from Test Image", predict),
]

for index, (text_val, command) in enumerate(buttons):
    row = index // 2
    col = index % 2
    btn = Button(button_frame, text=text_val, command=command, **button_style)
